A collection of programming assignments and practice work completed for **CS 31 – Introduction to Computer Science I (C++)** at **UCLA** during **Winter 2025**.

---

## 📚 Course Overview
**CS 31** is UCLA’s foundational programming course, focusing on:
- C++ programming fundamentals
- Algorithmic problem-solving
- Software design and debugging techniques

---

## 🛠️ Topics Covered
- **C++ Basics** → Variables, types, operators  
- **Control Flow** → If/else, switch, loops  
- **Functions** → Parameters, scope, return values  
- **Data Structures** → Arrays, C-strings  
- **Pointers & Memory Management** → `new`, `delete`  
- **File I/O** → Reading/writing files  
- **OOP** → Classes, constructors, destructors  
- **Debugging** → `gdb`, logical debugging

---

## 🚀 Running the Code
```bash
# Clone the repo
git clone https://github.com/aarnav-11/UCLA_CS31_W25_projects.git
cd UCLA_CS31_W25_projects

# Compile and run (example: project1)
g++ project1/main.cpp -o project1
./project1
